...
public interface SlProductMapper extends MyMapper<SlProduct> {
	...
	@Select("<script> select count(1) from sl_product a inner join sl_product_type c on a.pdtp_id=c.pdtp_id"
			+ " left join sl_file b on a.pd_icon = b.file_id where a.bd_id=#{bdId}  "
			+ "<when test='nameLike != null'> "
			+ " and a.pd_nm like #{nameLike} "
			+ "</when> "
	...
	Long getProductCount(@Param("bdId") String bdId, @Param("pdtpId") Long pdtpId, @Param("nameLike") String nameLike, @Param("enabled") Integer enabled);
