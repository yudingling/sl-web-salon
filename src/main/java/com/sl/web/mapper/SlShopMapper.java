...
public interface SlShopMapper extends MyMapper<SlShop> {
	...
	@Select("<script> select count(1) from sl_shop a inner join sl_user_shop x on a.shop_id=x.shop_id and x.role_id ='SHOP' inner join sl_user y on x.u_id = y.u_id "
			+ "<when test='bdId != null'> "
			+ " and a.bd_id=#{bdId} "
			+ "</when> "
			+ "<when test='nameLike != null'> "
	...
	Long getShopCount(@Param("bdId") String bdId, @Param("nameLike") String nameLike);
